...
__device__ VX3_DistributedNeuralController::VX3_DistributedNeuralController(VX3_VoxelyzeKernel* kernel, double* weights, int random_seed) {
  mlp = new VX3_MLP(NUM_SENSORS + NUM_SIGNALS, (NUM_SIGNALS / 2) + 2, weights);
...
__device__ void VX3_DistributedNeuralController::sense(VX3_Voxel* voxel, VX3_VoxelyzeKernel* kernel) {
  ...
  if ((kernel->min_x > voxel->pos.x) || (voxel->pos.x > kernel->max_x) || (voxel->pos.z > kernel->max_z)) {
    printf("(%d,%d,%d) is out-of-bounds with (%f,%f,%f) at %d\n", voxel->ix, voxel->iy, voxel->iz, voxel->pos.x, voxel->pos.y, voxel->pos.z, kernel->CurStepCount);
