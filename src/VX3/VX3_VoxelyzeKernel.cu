...
__global__ void gpu_update_temperature(VX3_Voxel *voxels, int num, double TempAmplitude, double TempPeriod, double currentTime, VX3_VoxelyzeKernel* k, VX3_DistributedNeuralController* controller) {
    ...
    if (gindex < num) {
        ...
        printf("temp for (%d,%d,%d) at %f neighbors {pos_x:%s neg_x:%s pos_y:%s neg_y:%s pos_z:%s neg_z:%s}: %f\n", t->ix, t->iy, t->iz, currentTime, currentTemperature, (t->adjacentVoxel((linkDirection)0)) ? t->adjacentVoxel((linkDirection)0)->pos : "na", (t->adjacentVoxel((linkDirection)1)) ? t->adjacentVoxel((linkDirection)1)->pos : "na", (t->adjacentVoxel((linkDirection)2)) ? t->adjacentVoxel((linkDirection)2)->pos : "na", (t->adjacentVoxel((linkDirection)3)) ? t->adjacentVoxel((linkDirection)3)->pos : "na", (t->adjacentVoxel((linkDirection)4)) ? t->adjacentVoxel((linkDirection)4)->pos : "na", (t->adjacentVoxel((linkDirection)5)) ? t->adjacentVoxel((linkDirection)5)->pos : "na");
