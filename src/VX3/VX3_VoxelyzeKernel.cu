...
__device__ void VX3_VoxelyzeKernel::computeFitness(VX3_DistributedNeuralController* controller) {
    ...
    double p_0 = (num_pos == 0) ? 0.0 : num_pos / voteStepCount;
    double p_1 = (num_neg == 0) ? 0.0 : num_neg / voteStepCount;
    double entropy = p_0 * log(p_0) + p_1 * log(p_1);
    printf("num_pos: %d num_neg: %d entropy: %f norm_entropy: %f\n", num_pos, num_neg, entropy, entropy / (0.5 * log(0.5) + 0.5 * log(0.5)));
    sensing_score += - entropy / (0.5 * log(0.5) + 0.5 * log(0.5));
    fitness_score = locomotion_score + sensing_score;
}
...
__global__ void gpu_update_temperature(VX3_Voxel *voxels, int num, double TempAmplitude, double TempPeriod, double currentTime, VX3_VoxelyzeKernel* k, VX3_DistributedNeuralController* controller) {
    ...
    if (gindex < num) {
        ...
        //printf("inputs for (%d,%d,%d) at %f: %f %f %f %f %f %f %f %f %f %f %f %f\n", t->ix, t->iy, t->iz, currentTime, t->inputs[0], t->inputs[1], t->inputs[2], t->inputs[3], t->inputs[4], t->inputs[5], t->inputs[6], t->inputs[7], t->inputs[8], t->inputs[9], t->inputs[10], t->inputs[11]);
