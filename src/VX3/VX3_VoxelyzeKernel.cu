...
__global__ void gpu_update_temperature(VX3_Voxel *voxels, int num, double TempAmplitude, double TempPeriod, double currentTime, VX3_VoxelyzeKernel* k, VX3_DistributedNeuralController* controller) {
    ...
    if (gindex < num) {
        ...
        printf("temp for (%d,%d,%d) at %f neighbors {pos_x:%s neg_x:%s pos_y:%s neg_y:%s pos_z:%s neg_z:%s}: %f\n", t->ix, t->iy, t->iz, currentTime, currentTemperature, (voxel->adjacentVoxel((linkDirection)0)) ? voxel->adjacentVoxel((linkDirection)0)->pos : "na", (voxel->adjacentVoxel((linkDirection)1)) ? voxel->adjacentVoxel((linkDirection)1)->pos : "na", (voxel->adjacentVoxel((linkDirection)2)) ? voxel->adjacentVoxel((linkDirection)2)->pos : "na", (voxel->adjacentVoxel((linkDirection)3)) ? voxel->adjacentVoxel((linkDirection)3)->pos : "na", (voxel->adjacentVoxel((linkDirection)4)) ? voxel->adjacentVoxel((linkDirection)4)->pos : "na", (voxel->adjacentVoxel((linkDirection)5)) ? voxel->adjacentVoxel((linkDirection)5)->pos : "na");
